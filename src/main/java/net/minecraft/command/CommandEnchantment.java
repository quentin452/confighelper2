                    ...
                    if (itemstack.hasTagCompound()) {
                        ...
                        if (nbttaglist != null) {
                            for (int k = 0; k < nbttaglist.tagCount(); ++k) {
                                int id = nbttaglist.getCompoundTagAt(k)
                                    .getInteger("id");
                                ...
                                if (Enchantment.enchantmentsList[id] != null) {
                                    ...
                                    if (!enchantment1.canApplyTogether(enchantment)
                                        || !enchantment.canApplyTogether(enchantment1)) // Forge BugFix: Let Both
                                                                                        // enchantments veto being
                                                                                        // together
                                    {
                                        throw new CommandException(
                                            "commands.enchant.cantCombine",
                                            new Object[] { enchantment.getTranslatedName(j),
                                                enchantment1.getTranslatedName(
                                                    nbttaglist.getCompoundTagAt(k)
                                                        .getInteger("lvl")) });
                                    }
                                }
                            }
