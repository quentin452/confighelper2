...
public class ASMHelper {
    ...
    public static Map<String, ClassNode> classNodes = new HashMap();// input to classNode gets cleared when a MCWriter
                                                                    // gets it's byte array
    ...
    public static ClassNode getClassNode(String input) throws IOException {
        InputStream stream = ASMHelper.class.getClassLoader()
    ...
    public static void patchInsns(MethodNode method, String classNew, String classOld, boolean patchStatic) {
        ...
        for (AbstractInsnNode ab : method.instructions.toArray()) {
            if (ab instanceof MethodInsnNode) {
                ...
                if (min.owner.equals(classOld) && (!isStaticMethod(min) || patchStatic)) min.owner = classNew;
            } else if (ab instanceof FieldInsnNode) {
                FieldInsnNode fin = (FieldInsnNode) ab;
                if (fin.owner.equals(classOld) && (!isStaticFeild(fin) || patchStatic)) fin.owner = classNew;
            }
        }
    }
    ...
    public static int getLocalVarIndex(MethodNode method, String owner, String name) {
        for (LocalVariableNode local : method.localVariables) {
            if (local.desc.equals(owner) && local.name.equals(name)) return local.index;
        }
        return -1;
    }
    ...
    public static int getLocalVarIndexByName(MethodNode method, String name) {
        for (LocalVariableNode local : method.localVariables) {
            if (local.name.equals(name)) return local.index;
        }
        return -1;
    }
    ...
    /**
     * add a object field to the class with optional signature. The paramDesc is a descriptor of the types of a class
     * HashMap<key,value>
     */
    ...
    public static void addIfMethod(ClassNode node, MethodNode method) {
        if (!containsMethod(node, method)) node.methods.add(method);
    }
    ...
    /**
     * remove a method don't remove ones that are going to get executed unless you immediately add the same method and
     * descriptor back
     * 
     * @throws IOException
     */
    ...
    public static LineNumberNode firstInsn(MethodNode method) {
        for (AbstractInsnNode obj : method.instructions.toArray())
            if (obj instanceof LineNumberNode) return (LineNumberNode) obj;
        return null;
    }
    ...
    public static AbstractInsnNode lastInsn(MethodNode method, int opCode) {
        ...
        for (int i = arr.length - 1; i >= 0; i--) {
            AbstractInsnNode ab = arr[i];
            if (ab.getOpcode() == opCode) return ab;
        }
        return null;
    }
    ...
    public static JumpInsnNode nextJumpInsn(AbstractInsnNode starting) {
        ...
        while (k != null) {
            k = k.getNext();
            if (k instanceof JumpInsnNode) return (JumpInsnNode) k;
        }
        return null;
    }
    ...
    public static FieldInsnNode nextFieldInsn(AbstractInsnNode starting) {
        ...
        while (k != null) {
            k = k.getNext();
            if (k instanceof FieldInsnNode) return (FieldInsnNode) k;
        }
        return null;
    }
    ...
    public static FieldInsnNode nextFieldInsn(AbstractInsnNode starting, FieldInsnNode compare) {
        ...
        while (k != null) {
            k = k.getNext();
            if (k instanceof FieldInsnNode && equals(compare, k)) return (FieldInsnNode) k;
        }
        return null;
    }
    ...
    public static MethodInsnNode nextMethodInsn(AbstractInsnNode starting, MethodInsnNode insn) {
        ...
        while (k != null) {
            k = k.getNext();
            if (k instanceof MethodInsnNode && equals(insn, k)) return (MethodInsnNode) k;
        }
        return null;
    }
    ...
    public static MethodInsnNode nextMethodInsn(AbstractInsnNode starting) {
        ...
        while (k != null) {
            k = k.getNext();
            if (k instanceof MethodInsnNode) return (MethodInsnNode) k;
        }
        return null;
    }
    ...
    public static IntInsnNode nextIntInsn(AbstractInsnNode ab) {
        while (ab != null) {
            ab = ab.getNext();
            if (ab instanceof IntInsnNode) return (IntInsnNode) ab;
        }
        return null;
    }
    ...
    public static AbstractInsnNode nextInsn(AbstractInsnNode ab, int opcode) {
        while (ab != null) {
            ab = ab.getNext();
            if (ab != null && ab.getOpcode() == opcode) return ab;
        }
        return null;
    }
    ...
    public static AbstractInsnNode nextLineNumber(AbstractInsnNode ab) {
        while (ab != null) {
            ab = ab.getNext();
            if (ab instanceof LineNumberNode) return ab;
        }
        return null;
    }
    ...
    public static AbstractInsnNode nextLabel(AbstractInsnNode ab) {
        while (ab != null) {
            ab = ab.getNext();
            if (ab instanceof LabelNode) return ab;
        }
        return null;
    }
    ...
    public static AbstractInsnNode getPushInsn(int value) throws IllegalArgumentException {
        if (value <= Byte.MAX_VALUE) return new IntInsnNode(Opcodes.BIPUSH, value);
        if (value <= Short.MAX_VALUE) return new IntInsnNode(Opcodes.SIPUSH, value);
    ...
    public static void removeLine(MethodNode method, AbstractInsnNode start) {
        if (!(start instanceof LineNumberNode)) start = ASMHelper.previousLabel(start)
            .getNext();
        if (start instanceof FrameNode) start = start.getNext();
        AbstractInsnNode end = ASMHelper.nextLabel(start)
            .getPrevious();// previous is label and the one before the label is the end
    ...
    public static void clearNextThrowable(MethodNode method, String desc_exception) {
        ...
        if (start != null) ASMHelper.removeInsn(method, start, end);
    }
    ...
    public static boolean equals(MethodInsnNode m1, AbstractInsnNode ab) {
        if (!(ab instanceof MethodInsnNode)) return false;
        MethodInsnNode m2 = (MethodInsnNode) ab;
        return m1.getOpcode() == m2.getOpcode() && m1.name.equals(m2.name)
            && m1.desc.equals(m2.desc)
            && m1.owner.equals(m2.owner)
            && m1.itf == m2.itf;
    }

    public static boolean equals(FieldInsnNode f1, AbstractInsnNode ab) {
        if (!(ab instanceof FieldInsnNode)) return false;
        FieldInsnNode f2 = (FieldInsnNode) ab;
        return f1.getOpcode() == f2.getOpcode() && f1.name.equals(f2.name)
            && f1.desc.equals(f2.desc)
            && f1.owner.equals(f2.owner);
    }

    public static boolean equals(LdcInsnNode l1, AbstractInsnNode ab) {
        if (!(ab instanceof LdcInsnNode)) return false;
