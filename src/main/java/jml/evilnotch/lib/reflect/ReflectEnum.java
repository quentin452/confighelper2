...
public class ReflectEnum {
    ...
    static {
        try {
            ...
            factory = ReflectionHandler.getMethod(classReflectFactory, "getReflectionFactory")
                .invoke(null);
            ...
            fieldAccessor = ReflectionHandler
                .getMethod(classReflectFactory, "newFieldAccessor", Field.class, boolean.class);
    ...
    public static Constructor getEnumConstructor(Class<? extends Enum> clazz, Class... params) {
        ...
        System.arraycopy(params, 0, corrected, 2, params.length);// copy the contents of params into corrected starting
                                                                 // at index 2
     ...
     */
    @Deprecated
    public static Object getCtrAccessor(Constructor ctr)
        throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
        return ctrAccessor.invoke(factory, ctr);
    }

    @Deprecated
    public static Object getFieldAcessor(Field field)
    ...
    public static void addEnum(Enum... enums) {
        try {
            ...
            for (Enum e : enums) setOrdinal(e, index++);
        ...
        for (Enum e : enums) {
            ...
            if (!e.getClass()
                ...
                throw new RuntimeException(
                    "enum class:" + clazz.getName() + " doesn't match the type of:" + e.getClass());
    ...
    private static Field getEnumHolder(Class<? extends Enum> clazz) {
        ...
        if (fieldValues == null) {
            int flags = (ObfHelper.isDeob ? Modifier.PUBLIC : Modifier.PRIVATE) | Modifier.STATIC
                | Modifier.FINAL
                | 0x1000 /* SYNTHETIC */;
            if (fieldValues == null) {
    ...
    private static void setField(Field f, Object instance, Object value)
        throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    ...
    private static Class[] getParams(Object[] params) {
        ...
        for (int i = 0; i < params.length; i++) c[i] = params[i].getClass();
        return c;
    }
    ...
    public static <T extends Enum<?>> void removeEnum(Enum... toRemove) {
        try {
            ...
            for (Enum eh : toRemove) oldValues[eh.ordinal()] = null;
            // create a new list that's trimed around it
            List<Enum> list = new ArrayList<Enum>();
            for (Enum old : oldValues) {
                if (old != null) list.add(old);
            }
            int index = 0;
            for (Enum e : list) setOrdinal(e, index++);
    ...
    private static void sanityCheckRemove(Enum[] toRemove) {
        for (Enum e : toRemove) {
            if (e.ordinal() < 0) throw new RuntimeException(
                "ordinal not set:" + e + " the enum is probably not in memory call EnumReflect#addEnum(enum)");
            Enum check = getEnum(e.getClass(), e.ordinal());
            if (check == null) throw new IllegalArgumentException("enum already removed:" + e);
            else if (check != e) throw new RuntimeException("enum mismatch:" + e + " index:" + check);
        }
    }
