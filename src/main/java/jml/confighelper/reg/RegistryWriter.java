...
public class RegistryWriter {
    ...
    public void write() {
        try {
            ...
            if (RegistryConfig.dumpIds) this.writeIds();
        } catch (Throwable t) {
            t.printStackTrace();
        } finally {
            this.reg.resetInfoIds();
        ...
        for (Integer id : this.reg.getOrgIds()) {
            if (reg.isConflicting(id)) {
                if (this.reg instanceof RegistryInt) {
                    ...
                    if (replacedIntReg(entries)) json.put("replaced", true);
                    ...
                    for (Registry.Entry entry : this.reg.getEntryOrg(id)) {
                        ...
                        if (entry.replaced) json.put("replaced", true);
                        if (entry.newId != entry.org) json.put("freeId", this.reg.getNextFreeId(entry.newId));
    ...
    protected void writeSuggestions() throws IOException {
        ...
        for (Registry.Entry entry : this.reg) {
            if (!canSuggest(this.reg, entry)) continue;
        ...
        for (Map.Entry<String, List<Registry.Entry>> map : entries.entrySet()) {
            ...
            if (hasModName) writer.write(modName + "\r\n");
            if (RegistryConfig.suggestIdChunks) {
                Set<Integer> ids = new TreeSet();
                for (Registry.Entry e : list) ids.add(this.reg.getNextSuggestedId(e.newId));// add all possible
                                                                                            // suggested ids in order
                Set<IdChunk> chunks = IdChunk.configureRanges(ids);// format them into id chunks
                for (IdChunk c : chunks) writer.write(c.toString() + "\r\n");
            } else {
            ...
            if (hasModName) writer.write("\r\n");
        }
        writer.close();
    }

    protected boolean canSuggest(Registry reg, Entry e) {
        if (e.replaced || e.obj instanceof BiomeGenMutated) return false;
