...
public class Registries {
    ...
    public static boolean isCrashing;// basically is in the process of crashing but, hasn't nessarly called
    ...
    public static CentralRegistry<ResourceLocation, WatcherDataType> datawatchertypes = new CentralRegistry<ResourceLocation, WatcherDataType>(
        DataType.DATAWATCHERTYPE);
    ...
    public static int registerBiome(BiomeGenBase biome, int id, boolean reg) {
        if (reg || !reg && RegistryConfig.regUnregBiomes) {
            if (initBiomes && RegistryConfig.autoBiomeMutated && biome instanceof BiomeGenMutated) id = nextBiome--;// automate
                                                                                                                    // BiomeGenMutated
                                                                                                                    // for
                                                                                                                    // mods
    ...
    public static void strictWatcher() {
        if (datawatchers != null) datawatchers.strict = true;
    }

    public static boolean hasConflicts() {
        return biomes.hasConflicts || potions.hasConflicts
    ...
    public static void writeWatcher() {
        if (Registries.datawatchers == null) return;
        try {
    ...
    public static void makeCrashReport(String cat, String msg, boolean onlyWatcher) {
        ...
        isCrashing = true;
        hasCrashed = true;
        if (!onlyWatcher) Registries.write();
        else Registries.writeWatcher();
    ...
    public static String getConflictTypes() {
        ...
        b.append('[');
        if (Registries.biomes.hasConflicts) b.append(Registries.biomes.dataType + ", ");
        if (Registries.potions.hasConflicts) b.append(Registries.potions.dataType + ", ");
        if (Registries.enchantments.hasConflicts) b.append(Registries.enchantments.dataType + ", ");
        if (Registries.dimensions.hasConflicts) b.append(Registries.dimensions.dataType + ", ");
        if (Registries.providers.hasConflicts) b.append(Registries.providers.dataType + ", ");
        if (Registries.entities.hasConflicts) b.append(Registries.entities.dataType + ", ");
    ...
    public static String getModName(String clazz) {
        if (clazz.startsWith("net.minecraft.")) return "Minecraft";
        if (packageOwners == null) {
            try {
                loadController = (LoadController) ReflectionHelper.findField(Loader.class, "modController")
                    .get(Loader.instance());
                packageOwners = (ListMultimap<String, ModContainer>) ReflectionHelper
                    .findField(LoadController.class, "packageOwners")
                    .get(loadController);
            } catch (Throwable t) {
                t.printStackTrace();
            }
            if (packageOwners == null) return "packageOwners-" + null;
        }
        String pakage = clazz.substring(0, JavaUtil.lastChar(clazz, '.'));
        if (packageOwners.containsKey(pakage)) {
            ModContainer mod = packageOwners.get(pakage)
                .get(0);
            if (mod.getModId()
                .equals("examplemod")) {
    ...
    public static int guessDimOrgId(int providerId) {
        for (Registry.Entry e : dimensions) {
            ...
            if (providerId == dim.providerNewId) return e.org;
        }
    ...
    public static void loadComplete() {
        ...
        Registries.enchantments.securityCheck();// security check static[] capeable registries again to ensure registry
                                                // saftey in case something screwed it up in post init
    }
